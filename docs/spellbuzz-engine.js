/* SpellBuzz Spelling World — lesson engine, profile, adaptive, coach */
(function (global) {
  "use strict";

  const PROFILE_KEY = "spellbuzz_profile";
  ...
  function defaultProfile() {
    return {
      ...
      xp: 0,
      stars: 0,
      unlockedWorlds: ["letter-sound-camp"],
      nodesCompleted: {},
      lessonsCompleted: {},
  ...
  function weakestSkills(profile, limit) {
    const labels = (global.SBContent && global.SBContent.SKILL_LABELS) || {};
  ...
  function countWorldNodesDone(profile, worldId) {
    const world = global.SBContent && global.SBContent.getWorld(worldId);
    if (!world) return 0;
    let n = 0;
  ...
  function refreshUnlocks(profile) {
    ...
    const content = global.SBContent;
    if (!content) return;
  ...
  function getGradeBandConfig(grade) {
    const bands = (global.SBContent && global.SBContent.GRADE_BANDS) || [];
  ...
  function worldIdForGrade(grade) {
    ...
    return band ? band.worldId : "letter-sound-camp";
  }
  ...
  function nodeUnlocked(profile, worldId, nodeId, nodeIndex, options) {
    ...
    const world = global.SBContent.getWorld(worldId);
    if (!world) return false;
  ...
  function markNodeCompleteIfReady(profile, worldId, nodeId) {
    const node = global.SBContent.getNode(worldId, nodeId);
  ...
  function availableCosmetics(profile) {
    const list = (global.SBContent && global.SBContent.COSMETICS) || [];
    return list.map((c) => ({
      id: c.id,
      name: c.name,
      cost: c.cost,
  ...
  function unlockCosmetic(profile, cosmeticId) {
    const item = ((global.SBContent && global.SBContent.COSMETICS) || []).find((c) => c.id === cosmeticId);
  ...
  function queueRemediation(profile, skill) {
    const bank = global.SBContent && global.SBContent.REMEDIATION_BANK[skill];
  ...
  function injectAdaptiveActivities(lesson, profile) {
    ...
    weak.forEach((w) => {
      const bank = global.SBContent.REMEDIATION_BANK[w.skill];
      if (!bank || !bank.length) return;
  ...
  function coachMessage(profile, context, extra) {
    ...
    if (context === "correct") {
      if ((extra.streak || 0) >= 3) return "Great streak! Great streak! Your spelling brain is warming up.";
    ...
    if (context === "ask") {
      return weak
        ? "Try a short practice on " + weak.label + ". Say each sound, then write the word."
        : "Pick a world at your level and finish one spelling lesson — small steps add up!";
    }
    return "I'm your spelling coach. Listen, sound it out, and grow!";
  }
